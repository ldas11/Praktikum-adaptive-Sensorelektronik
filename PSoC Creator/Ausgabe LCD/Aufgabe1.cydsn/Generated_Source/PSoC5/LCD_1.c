...
static void LCD_1_WrDatNib(uint8 nibble) ;
static void LCD_1_WrCntrlNib(uint8 nibble) ;
...
{
    LCD_1_WriteData((uint8)character);
}
...
void LCD_1_IsReady(void) 
{
    uint8 value;
    uint32 timeout;
    timeout = LCD_1_READY_DELAY;

    /* Clear LCD port*/
    LCD_1_PORT_DR_REG &= ((uint8)(~LCD_1_PORT_MASK));
    ...
    */
        ...
        value = LCD_1_PORT_PC_REG & ((uint32) (~ LCD_1_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_1_PORT_PC_REG = value | LCD_1_HIGH_Z_DATA_DM;
        ...
        /* Mask off data pins to clear old values out */
        value = LCD_1_PORT_DM0_REG & ((uint8)(~LCD_1_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_1_PORT_DM0_REG = value | (LCD_1_HIGH_Z_DM0 & LCD_1_DATA_MASK);

        /* Mask off data pins to clear old values out */
        value = LCD_1_PORT_DM1_REG & ((uint8)(~LCD_1_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_1_PORT_DM1_REG = value;

        /* Mask off data pins to clear old values out */
        value = LCD_1_PORT_DM2_REG & ((uint8)(~LCD_1_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_1_PORT_DM2_REG = value;
    
    #endif /* CY_PSOC4 */
    
    /* Make sure RS is low */
    LCD_1_PORT_DR_REG &= ((uint8)(~LCD_1_RS));
    ...
    do
    {
        ...
        /* Set enable low */
        LCD_1_PORT_DR_REG &= ((uint8)(~LCD_1_E));
    ...
    LCD_1_PORT_DR_REG &= ((uint8)(~LCD_1_RW));
    ...
    LCD_1_PORT_DR_REG &= ((uint8)(~LCD_1_PORT_MASK));
        ...
        value = LCD_1_PORT_PC_REG & ((uint32)(~ LCD_1_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_1_PORT_PC_REG = value | LCD_1_STRONG_DATA_DM;
        ...
        */
        LCD_1_PORT_DM0_REG &= ((uint8)(~LCD_1_DATA_MASK));
...
static void LCD_1_WrDatNib(uint8 nibble) 
{
    ...
    /* Clear data pins */
    LCD_1_PORT_DR_REG &= ((uint8)(~LCD_1_DATA_MASK));

    /* Write data, bring E high */
    #if(0u != LCD_1_PORT_SHIFT) /* MISRA forbids shift by 0 so need to handle that */
        LCD_1_PORT_DR_REG |= 
            (LCD_1_E | ((uint8)(((uint8) nibble) << LCD_1_PORT_SHIFT)));
    #else
        LCD_1_PORT_DR_REG |= (LCD_1_E | nibble);
    #endif /* (0u != LCD_1_PORT_SHIFT) */
...
static void LCD_1_WrCntrlNib(uint8 nibble) 
{
    ...
    LCD_1_PORT_DR_REG &= ((uint8)(~(LCD_1_RS | LCD_1_RW)));
    ...
    *******************************************************************************/
    ...
    {
        static char8 const CYCODE LCD_1_hex[16u] = "0123456789ABCDEF";
    ...
    /*******************************************************************************
    *  Function Name: LCD_1_PrintInt16
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        LCD_1_PrintInt8((uint8)(value >> LCD_1_U16_UPPER_BYTE_SHIFT));
        LCD_1_PrintInt8((uint8)(value & LCD_1_U16_LOWER_BYTE_MASK));
    }

    
    /*******************************************************************************
    *  Function Name: LCD_1_PrintInt32
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 shift = LCD_1_32_BIT_SHIFT;
        ...
        while (shift != 0u)
        {
            ...
            */
            shift -= LCD_1_8_BIT_SHIFT;
            
            /* Print 8 bits of uint32 hex number */
            LCD_1_PrintInt8((uint8) ((uint32) (value >> shift)));
        }
    }
    
    
    /*******************************************************************************
    *  Function Name: LCD_1_PrintNumber
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    *  Function Name: LCD_1_PrintU32Number
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 tmpDigit;
        char8 number[LCD_1_NUMBER_OF_REMAINDERS_U32 + 1u];
        uint8 digIndex = LCD_1_NUMBER_OF_REMAINDERS_U32;
        ...
        */
        ...
        while(value >= LCD_1_TEN)
        {
            ...
            */
            tmpDigit = (uint8) (((uint8) (value % LCD_1_TEN)) + (uint8) LCD_1_ZERO_CHAR_ASCII);
        ...
        */
        ...
        /* Print out number */
        LCD_1_PrintString(&number[digIndex]);
    }
