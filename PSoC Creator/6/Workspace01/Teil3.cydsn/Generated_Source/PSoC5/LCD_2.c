...
static void LCD_2_WrDatNib(uint8 nibble) ;
static void LCD_2_WrCntrlNib(uint8 nibble) ;

/* Stores state of component. Indicates whether component is or not
* in enable state.
*/
uint8 LCD_2_enableState = 0u;
...
void LCD_2_Init(void) 
{
    ...
    LCD_2_WrCntrlNib(LCD_2_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(5u);                                                         /* Delay 5 ms */
    LCD_2_WrCntrlNib(LCD_2_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(15u);                                                        /* Delay 15 ms */
    LCD_2_WrCntrlNib(LCD_2_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(1u);                                                         /* Delay 1 ms */
    LCD_2_WrCntrlNib(LCD_2_DISPLAY_4_BIT_INIT);    /* Selects 4-bit mode */
    CyDelay(5u);                                                         /* Delay 5 ms */

    LCD_2_WriteControl(LCD_2_CURSOR_AUTO_INCR_ON);    /* Incr Cursor After Writes */
    LCD_2_WriteControl(LCD_2_DISPLAY_CURSOR_ON);      /* Turn Display, Cursor ON */
    LCD_2_WriteControl(LCD_2_DISPLAY_2_LINES_5x10);   /* 2 Lines by 5x10 Characters */
    LCD_2_WriteControl(LCD_2_DISPLAY_CURSOR_OFF);     /* Turn Display, Cursor OFF */
    LCD_2_WriteControl(LCD_2_CLEAR_DISPLAY);          /* Clear LCD Screen */
    LCD_2_WriteControl(LCD_2_DISPLAY_ON_CURSOR_OFF);  /* Turn Display ON, Cursor OFF */
    LCD_2_WriteControl(LCD_2_RESET_CURSOR_POSITION);  /* Set Cursor to 0,0 */
    CyDelay(5u);

    #if(LCD_2_CUSTOM_CHAR_SET != LCD_2_NONE)
        LCD_2_LoadCustomFonts(LCD_2_customFonts);
    #endif /* LCD_2_CUSTOM_CHAR_SET != LCD_2_NONE */
}
...
*  LCD_2_Init() turns on the LCD.
*
*******************************************************************************/
void LCD_2_Enable(void) 
{
    LCD_2_DisplayOn();
    LCD_2_enableState = 1u;
}
...
void LCD_2_Position(uint8 row, uint8 column) 
{
    ...
    {
        case (uint8)0:
            LCD_2_WriteControl(LCD_2_ROW_0_START + column);
            break;
        case (uint8) 1:
            LCD_2_WriteControl(LCD_2_ROW_1_START + column);
            break;
        case (uint8) 2:
            LCD_2_WriteControl(LCD_2_ROW_2_START + column);
            break;
        case (uint8) 3:
            LCD_2_WriteControl(LCD_2_ROW_3_START + column);
            break;
        default:
...
/*******************************************************************************
* Function Name: LCD_2_PrintString
********************************************************************************
*
* Summary:
...
{
    LCD_2_WriteData((uint8)character);
}
...
void LCD_2_WriteData(uint8 dByte) 
{
    ...
    LCD_2_IsReady();
    nibble = dByte >> LCD_2_NIBBLE_SHIFT;

    /* Write high nibble */
    LCD_2_WrDatNib(nibble);

    nibble = dByte & LCD_2_NIBBLE_MASK;
    /* Write low nibble */
    LCD_2_WrDatNib(nibble);
}
...
/*******************************************************************************
*  Function Name: LCD_2_WriteControl
********************************************************************************
*
* Summary:
...
void LCD_2_WriteControl(uint8 cByte) 
{
    ...
    nibble = cByte >> LCD_2_NIBBLE_SHIFT;
    
    /* WrCntrlNib(High Nibble) */
    LCD_2_WrCntrlNib(nibble);
    nibble = cByte & LCD_2_NIBBLE_MASK;
...
void LCD_2_IsReady(void) 
{
    uint8 value;
    uint32 timeout;
    timeout = LCD_2_READY_DELAY;

    /* Clear LCD port*/
    LCD_2_PORT_DR_REG &= ((uint8)(~LCD_2_PORT_MASK));
    ...
    */
        ...
        value = LCD_2_PORT_PC_REG & ((uint32) (~ LCD_2_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_2_PORT_PC_REG = value | LCD_2_HIGH_Z_DATA_DM;
        ...
        /* Mask off data pins to clear old values out */
        value = LCD_2_PORT_DM0_REG & ((uint8)(~LCD_2_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_2_PORT_DM0_REG = value | (LCD_2_HIGH_Z_DM0 & LCD_2_DATA_MASK);

        /* Mask off data pins to clear old values out */
        value = LCD_2_PORT_DM1_REG & ((uint8)(~LCD_2_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_2_PORT_DM1_REG = value;

        /* Mask off data pins to clear old values out */
        value = LCD_2_PORT_DM2_REG & ((uint8)(~LCD_2_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_2_PORT_DM2_REG = value;
    
    #endif /* CY_PSOC4 */
    
    /* Make sure RS is low */
    LCD_2_PORT_DR_REG &= ((uint8)(~LCD_2_RS));

    /* Set R/W high to read */
    LCD_2_PORT_DR_REG |= LCD_2_RW;
    ...
    LCD_2_PORT_DR_REG &= ((uint8)(~LCD_2_RW));
    ...
    LCD_2_PORT_DR_REG &= ((uint8)(~LCD_2_PORT_MASK));
        ...
        value = LCD_2_PORT_PC_REG & ((uint32)(~ LCD_2_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_2_PORT_PC_REG = value | LCD_2_STRONG_DATA_DM;
        ...
        */
        LCD_2_PORT_DM0_REG &= ((uint8)(~LCD_2_DATA_MASK));
        /* Mask off data pins to clear high z values out */
        value = LCD_2_PORT_DM1_REG & ((uint8)(~LCD_2_DATA_MASK));
        /* Configure data pins to Strong Drive, others unchanged */
        LCD_2_PORT_DM1_REG = value | (LCD_2_STRONG_DM1 & LCD_2_DATA_MASK);

        /* Mask off data pins to clear high z values out */
        value = LCD_2_PORT_DM2_REG & ((uint8)(~LCD_2_DATA_MASK));
        /* Configure data pins to Strong Drive, others unchanged */
        LCD_2_PORT_DM2_REG = value | (LCD_2_STRONG_DM2 & LCD_2_DATA_MASK);
...
static void LCD_2_WrDatNib(uint8 nibble) 
{
    ...
    /* Clear data pins */
    LCD_2_PORT_DR_REG &= ((uint8)(~LCD_2_DATA_MASK));

    /* Write data, bring E high */
    #if(0u != LCD_2_PORT_SHIFT) /* MISRA forbids shift by 0 so need to handle that */
        LCD_2_PORT_DR_REG |= 
            (LCD_2_E | ((uint8)(((uint8) nibble) << LCD_2_PORT_SHIFT)));
    #else
        LCD_2_PORT_DR_REG |= (LCD_2_E | nibble);
    #endif /* (0u != LCD_2_PORT_SHIFT) */
...
/*******************************************************************************
*  Function Name: LCD_2_WrCntrlNib
********************************************************************************
*
* Summary:
...
static void LCD_2_WrCntrlNib(uint8 nibble) 
{
    ...
    LCD_2_PORT_DR_REG &= ((uint8)(~(LCD_2_RS | LCD_2_RW)));
    ...
    *******************************************************************************/
    ...
    {
        static char8 const CYCODE LCD_2_hex[16u] = "0123456789ABCDEF";
        
        LCD_2_PutChar((char8) LCD_2_hex[value >> LCD_2_BYTE_UPPER_NIBBLE_SHIFT]);
        LCD_2_PutChar((char8) LCD_2_hex[value & LCD_2_BYTE_LOWER_NIBBLE_MASK]);
    }


    /*******************************************************************************
    *  Function Name: LCD_2_PrintInt16
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        LCD_2_PrintInt8((uint8)(value >> LCD_2_U16_UPPER_BYTE_SHIFT));
        LCD_2_PrintInt8((uint8)(value & LCD_2_U16_LOWER_BYTE_MASK));
    }

    
    /*******************************************************************************
    *  Function Name: LCD_2_PrintInt32
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 shift = LCD_2_32_BIT_SHIFT;
        ...
        while (shift != 0u)
        {
            ...
            */
            shift -= LCD_2_8_BIT_SHIFT;
            
            /* Print 8 bits of uint32 hex number */
            LCD_2_PrintInt8((uint8) ((uint32) (value >> shift)));
        }
    }
    
    
    /*******************************************************************************
    *  Function Name: LCD_2_PrintNumber
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    *  Function Name: LCD_2_PrintU32Number
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 tmpDigit;
        char8 number[LCD_2_NUMBER_OF_REMAINDERS_U32 + 1u];
        uint8 digIndex = LCD_2_NUMBER_OF_REMAINDERS_U32;
        ...
        */
        ...
        while(value >= LCD_2_TEN)
        {
            /* Extract decimal digit, indexed by 'digIndex', from 'value' and
            * convert it to ASCII character.
            */
            tmpDigit = (uint8) (((uint8) (value % LCD_2_TEN)) + (uint8) LCD_2_ZERO_CHAR_ASCII);
        ...
        */
        ...
        /* Print out number */
        LCD_2_PrintString(&number[digIndex]);
    }
